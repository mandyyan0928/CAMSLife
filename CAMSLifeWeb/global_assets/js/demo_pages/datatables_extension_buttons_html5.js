...
function getFileName() {
    if (typeof generateFileName === 'function') {
        // Check if a page-specific generateFileName function exists
        return generateFileName(); // Call the page-specific generateFileName function
    } else {
        return generateGeneralFileName(); // Call the general generateFileName function
    }
}

function generateGeneralFileName() {
    // Your general logic to generate the filename
    var currentDate = new Date().toISOString().slice(0, 10); // Example: Current date
    var fileName = 'myTable_export_' + currentDate;
    return fileName;
}